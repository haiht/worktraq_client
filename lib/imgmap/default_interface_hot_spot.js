...
function gui_row_select(id, setfocus, multiple) {
	...
	myimgmap.areas[id].ahref = product[id].product_url;
	myimgmap.areas[id].lastInput = props[id].getElementsByTagName('input')[2].value; //product[id].map_coords;
	myimgmap.areas[id].shape = props[id].getElementsByTagName('select')[0].value; //product[id].map_shape;
...
function gui_addArea_bak(id) {
	//var id = props.length;
	//id = 1;
	//alert(id + ' -- '+product.length + ' --- ' + myimgmap.areas.length);
	if(id=='undefined') return;
	if(id >= product.length && product[id]=='undefined'){ 
		//myimgmap.removeArea(id);
		return;	
	}
	var p = -1;
	for(var i = 0; i<product.length; i++){
		if(product[i].map_area == id && product[i].status==0){ 
			p = i;
			i = product.length;
		}
	}
	...
	var temp = '<input type="hidden" size="10"  name="img_id" class="img_id" data-id="'+product[p].product_id+'" value="' + id + '" readonly="1"/>';
	//temp+= '<input type="checkbox" name="img_active" class="img_active" id="img_active_'+id+'" value="'+id+'">';
	//could be checkbox in the future
	temp+= '<input type="radio" name="img_active" class="img_active" id="img_active_'+id+'" value="'+id+'">';
	temp+= '<select name="img_shape" class="img_shape">';
	temp+= '<option value="rect"'+(product[p].map_shape=='rect'?' selected="selected"':'')+'>rectangle</option>';
	temp+= '<option value="circle"'+(product[p].map_shape=='circle'?' selected="selected"':'')+'>circle</option>';
	temp+= '<option value="poly"'+(product[p].map_shape=='poly'?' selected="selected"':'')+'>polygon</option>';
	//temp+= '<option value="bezier1">bezier</option>';
	temp+= '</select><br />';
	temp+= 'Coords: <input type="text" name="img_coords" class="img_coords" value="'+product[p].map_coords+'">';
	temp+= '<input type="hidden" name="img_href" class="img_href" value="'+product[p].product_url+'">';
	temp+= '<input type="hidden" name="img_alt" class="img_alt" value="'+product[p].product_name+'">';
	temp+= '<select name="img_target" class="img_target" style="display:none">';
	temp+= '<option value="_self" selected="selected">this window</option>';
	temp+= '<option value="_blank" >new window</option>';
	temp+= '<option value="_top"   >top window</option>';
	temp+= '</select>';
	temp+= '<br /><img style="max-width:180" src="'+product[p].product_image+'" /><br />'+product[p].product_name+'<br />';
...
function Product(product_id, product_name, product_image, product_url, shape, coords, map_key, is_new){
	product_id = parseInt(product_id, 10);
	if(isNaN(product_id) || product_id < 0) product_id = 0;
	this.product_id = product_id;
	this.product_image = product_image;
	this.product_name = product_name;
	this.product_url = product_url;
	this.map_shape = shape;
...
function Map(product_id, product_name, product_image, product_url, shape, coords, map_key){
	this.product_id = product_id;
	this.product_image = product_image;
	this.product_name = product_name;
	this.product_url = product_url;
