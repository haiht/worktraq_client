...
function imgmap(config) {
	 ...
	 */
	this.config.mode     = "editor2";
	this.config.maxArea = 0;//0 is not retrict
	this.config.areaText = [];
	this.config.areaAlt = [];
	...
	this.config.label_style        = 'font-weight: normal; font-size: 11px; font-family: Arial; color: #FFF';
 ...
 *	@see	#initArea
 */
imgmap.prototype.addNewArea = function(p) {
		//alert(l);
		if(p=='undefined') return;
		//alert(typeof(p) +' '+this.viewmode);
		if (this.viewmode === 1) {return;}//exit if preview mode
		var lastarea = this._getLastArea();
		var id = (lastarea) ? lastarea.aid + 1 : 0;
		//var id = iid;
		//alert(id);
		//if(this.config.maxArea >0 && id>= this.config.maxArea) return;
		//alert(this.config.label);
		...
		if(typeof(p)!='undefined'){
			this.areas[id].shape = p.map_shape;
			this.areas[id].lastInput = p.map_coords;
			this.areas[id].ahref = p.product_url;
			p.map_area = id;
		}
		/*End edit*/
		...
		if(typeof(p)!='undefined'){
			this.initArea(id, this.areas[id].shape);
			if(p.map_coords!=''){
				this._recalculate(id, p.map_coords);
...
imgmap.prototype.initArea = function(id, shape) {
	...
	//this.areas[id].ahref     = '';
	this.areas[id].ahref     = this.config.areaText[id];
	
	this.areas[id].atitle    = '';
	this.areas[id].aalt      = this.config.areaAlt[id];
...
imgmap.prototype._putlabel = function(id) {
	...
	try {
		...
		this.assignCSS(this.areas[id].label,  this.config.labelStyle[id])
	}
	catch (err) {
