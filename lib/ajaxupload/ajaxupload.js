...
(function($)
{
		...
		xhr.onreadystatechange=function()
		{
			if(this.readyState == 4 && this.status == 200)
			{
				try
				{
					...
					{
						settings.SLOTS--;
						settings.received = ret.received;
						queued.end(ret.name, ret.size, ret.status, ret.info, ret.received);
					}
    ...
    function ajaxList(list, obj, settings)
    {
    	...
    	obj.tools=tools;
    	obj.end = function(name, size, status, info, received)
	    {
	    	settings.UPLOADED++;
	    	...
	    	settings.success(name, received);
    ...
    {   	
    	if(FILES.length>0)
    	{
    		$('#ax-main-frame').unbind('load').bind('load',function(){
	    		...
	    		obj.progress(100);
	    		obj.end(ret.name, ret.size, ret.status, ret.info, ret.received);
    ...
    var globalSettings = 
    {
    	...
    	success:		function(fileName,retrieve){},		//function that triggers every time a file is uploaded
