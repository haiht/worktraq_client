...
class cls_tb_template{
	...
	public function __construct(MongoDB $db, $p_log_dir = ""){
		...
		$this->collection = $db->selectCollection('tb_template');
	...
	/**
	 * function return properties "template_default" value
	 * @return string value
	 */
	public function get_template_default(){
		return $this->v_template_default;
	}

	
	/**
	 * function allow change properties "template_default" value
	 * @param $p_template_default: string value
	 */
	public function set_template_default($p_template_default){
		$this->v_template_default = $p_template_default;
	}
    ...
    public function get_element(){
        return $this->v_element ;
    }

    /**
     * @param $arr_element
     */
    public function set_element($arr_element){
        $this->v_element = $arr_element;
    }
    ...
    public function get_template_file(){
        return $this->v_template_file;
    }

    /**
     * @param $p_temlate_file
     */
    public function set_template_file($p_temlate_file){
        $this->v_template_file = $p_temlate_file;
    }
	...
	public function insert(){
		$arr = array('template_id' => $this->v_template_id
					...
					,'template_default' => $this->v_template_default
	 ...
	 * @example:
	 * <code>
	 *       SELECT * FROM `tb_template` WHERE `user_id`=2 ORDER BY `user_email` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_template($db)
	...
	public function select_one(array $arr_where = array(), array $arr_order = array()){
		...
		foreach($rss as $arr){
			...
			$this->v_template_default = isset($arr['template_default'])?$arr['template_default']:'';
			$this->v_date_created = isset($arr['date_created'])?$arr['date_created']:(new MongoDate(time()));
			$this->v_element = isset($arr['element'])?$arr['element']:array();
            $this->v_template_file =  isset($arr['template_file'])?$arr['template_file']:'';
	 ...
	 * @example: 
	 * <code>
	 * SELECT `template_id` FROM `tb_template` WHERE `user_id`=2 ORDER BY `user_email` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_template($db)
	 ...
	 * @example: 
	 * <code>
	 *   SELECT `template_id` FROM `tb_template` WHERE `user_id`=2 ORDER BY `template_id` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_template($db)
	...
	public function select_distinct($p_field_name){
		return $this->collection->command(array("distinct"=>"tb_template", "key"=>$p_field_name));
	}
	...
	public function update(array $arr_where = array()){
		...
		if(isset($v_has_mongo_id) && $v_has_mongo_id)
			$arr = array('$set' => array('template_id' => $this->v_template_id,'template_name' => $this->v_template_name,'template_default' => $this->v_template_default,'date_created' => $this->v_date_created));
		 else 
			$arr = array('$set' => array('template_name' => $this->v_template_name,'template_default' => $this->v_template_default,'date_created' => $this->v_date_created));
		try{
