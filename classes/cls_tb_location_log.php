...
class cls_tb_location_log{
	...
	private $v_open_date = '0000-00-00 00:00:00';
	private $v_close_date = '0000-00-00 00:00:00';
	private $v_status = 0;
	...
	public function __construct(MongoDB $db, $p_log_dir = ""){
		...
		$this->collection = $db->selectCollection('tb_location_log');
	...
	public function set_open_date($p_open_date){
        if($p_open_date=='') $p_open_date = NULL;
        if(!is_null($p_open_date)){
            try{
	...
	public function set_close_date($p_close_date){
        if($p_close_date=='') $p_close_date = NULL;
        if(!is_null($p_close_date)){
            try{
	 ...
	 * @example:
	 * <code>
	 *       SELECT * FROM `tb_location_log` WHERE `user_id`=2 ORDER BY `user_email` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_location_log($db)
	...
	public function select_one(array $arr_where = array(), array $arr_order = array()){
		...
		foreach($rss as $arr){
			...
			$this->v_close_date = isset($arr['close_date'])?$arr['close_date']:(new MongoDate(time()));
	 ...
	 * @example: 
	 * <code>
	 * SELECT `log_id` FROM `tb_location_log` WHERE `user_id`=2 ORDER BY `user_email` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_location_log($db)
	 ...
	 * @example: 
	 * <code>
	 *   SELECT `log_id` FROM `tb_location_log` WHERE `user_id`=2 ORDER BY `log_id` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_location_log($db)
	...
	public function select_distinct($p_field_name){
		return $this->collection->command(array("distinct"=>"tb_location_log", "key"=>$p_field_name));
	}
	...
	public function update(array $arr_where = array()){
		...
		if(isset($v_has_mongo_id) && $v_has_mongo_id)
			$arr = array('$set' => array('log_id' => $this->v_log_id,'company_id' => $this->v_company_id,'location_id' => $this->v_location_id,'region_id' => $this->v_region_id,'region_contact' => $this->v_region_contact,'open_date' => $this->v_open_date,'close_date' => $this->v_close_date,'status' => $this->v_status));
		 else 
			$arr = array('$set' => array('company_id' => $this->v_company_id,'location_id' => $this->v_location_id,'region_id' => $this->v_region_id,'region_contact' => $this->v_region_contact,'open_date' => $this->v_open_date,'close_date' => $this->v_close_date,'status' => $this->v_status));
		try{
	...
	public function delete(array $arr_where = array()){
		...
		try{
			$this->collection->remove($arr_where, array('safe'=>true, 'multiple'=>true));
			return true;
		}catch(MongoCursorException $e){
			$this->v_error_code = $e->getCode();
			$this->v_error_message = $e->getMessage();
