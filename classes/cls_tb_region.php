...
class cls_tb_region{
	...
	private $v_region_id = 0;
	private $v_company_id = 0;
	private $v_region_name = '';
	private $v_region_type = 0;
	private $v_region_contact = 0;
	private $v_status = 0;
	...
	public function __construct(MongoDB $db, $p_log_dir = ""){
		...
		$this->collection = $db->selectCollection('tb_region');
		$this->collection->ensureIndex(array("company_id"=>1));
		$this->collection->ensureIndex(array("region_id"=>1), array('name'=>"region_id_key", "unique"=>1, "dropDups" => 1));
	}
	...
	/**
	 * function return properties "region_id" value
	 * @return int value
	 */
	public function get_region_id(){
		return (int) $this->v_region_id;
	}

	
	/**
	 * function allow change properties "region_id" value
	 * @param $p_region_id: int value
	 */
	public function set_region_id($p_region_id){
		$this->v_region_id = (int) $p_region_id;
	}
	...
	/**
	 * function return properties "region_name" value
	 * @return string value
	 */
	...
	/**
	 * function allow change properties "region_name" value
	 * @param $p_region_name: string value
	 */
	public function set_region_name($p_region_name){
		$this->v_region_name = $p_region_name;
	}

	
	/**
	 * function return properties "region_type" value
	 * @return int value
	 */
	public function get_region_type(){
		return (int) $this->v_region_type;
	}

	
	/**
	 * function allow change properties "region_type" value
	 * @param $p_region_type: int value
	 */
	public function set_region_type($p_region_type){
		$this->v_region_type = (int) $p_region_type;
	}

	
	/**
	 * function return properties "region_contact" value
	 * @return int value
	 */
	public function get_region_contact(){
		return (int) $this->v_region_contact;
	}

	
	/**
	 * function allow change properties "region_contact" value
	 * @param $p_region_contact: int value
	 */
	public function set_region_contact($p_region_contact){
		$this->v_region_contact = (int) $p_region_contact;
	}
	...
	public function insert(){
		$arr = array('region_id' => $this->v_region_id
					,'company_id' => $this->v_company_id
					,'region_name' => $this->v_region_name
					,'region_type' => $this->v_region_type
					,'region_contact' => $this->v_region_contact
	 ...
	 * @example:
	 * <code>
	 *       SELECT * FROM `tb_region` WHERE `user_id`=2 ORDER BY `user_email` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_region($db)
	...
	public function select_one(array $arr_where = array(), array $arr_order = array()){
		...
		$v_count = 0;
		foreach($rss as $arr){
			$this->v_region_id = isset($arr['region_id'])?$arr['region_id']:0;
			$this->v_company_id = isset($arr['company_id'])?$arr['company_id']:0;
			$this->v_region_name = isset($arr['region_name'])?$arr['region_name']:'';
			$this->v_region_type = isset($arr['region_type'])?$arr['region_type']:0;
			$this->v_region_contact = isset($arr['region_contact'])?$arr['region_contact']:0;
	 ...
	 * @example: 
	 * <code>
	 * SELECT `region_id` FROM `tb_region` WHERE `user_id`=2 ORDER BY `user_email` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_region($db)
	 * 		 $cls->select_scalar('region_id',array('user_id'=>2), array('user_email'=>-1))
	 * </code>
	 * @return mixed
	 */
	 ...
	 * @example: 
	 * <code>
	 *   SELECT `region_id` FROM `tb_region` WHERE `user_id`=2 ORDER BY `region_id` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_region($db)
	 * 		 $cls->select_next('region_id',array('user_id'=>2), array('region_id'=>-1))
	 * </code>
	 * @return int
	 */
	...
	public function select_distinct($p_field_name){
		return $this->collection->command(array("distinct"=>"tb_region", "key"=>$p_field_name));
	}
	...
	public function update(array $arr_where = array()){
		...
		if(isset($v_has_mongo_id) && $v_has_mongo_id)
			$arr = array('$set' => array('region_id' => $this->v_region_id,'company_id' => $this->v_company_id,'region_name' => $this->v_region_name,'region_type' => $this->v_region_type,'region_contact' => $this->v_region_contact,'status' => $this->v_status));
		 else 
			$arr = array('$set' => array('company_id' => $this->v_company_id,'region_name' => $this->v_region_name,'region_type' => $this->v_region_type,'region_contact' => $this->v_region_contact,'status' => $this->v_status));
		try{
