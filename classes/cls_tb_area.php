...
class cls_tb_area{
	...
	private $v_area_name = '';
	private $v_area_description = '';
	private $v_status = 0;
	private $v_area_order = 0;
	private $v_area_code = '';
	...
	public function __construct(MongoDB $db, $p_log_dir = ""){
		...
		$this->collection = $db->selectCollection('tb_area');
		$this->collection->ensureIndex(array("area_id"=>1), array('name'=>"area_id_key", "unique"=>1, "dropDups" => 1));
	}
	...
	/**
	 * function return properties "area_name" value
	 * @return string value
	 */
	...
	/**
	 * function allow change properties "area_name" value
	 * @param $p_area_name: string value
	 */
	public function set_area_name($p_area_name){
		$this->v_area_name = $p_area_name;
	}

	
	/**
	 * function return properties "area_description" value
	 * @return string value
	 */
	public function get_area_description(){
		return $this->v_area_description;
	}

	
	/**
	 * function allow change properties "area_description" value
	 * @param $p_area_description: string value
	 */
	public function set_area_description($p_area_description){
		$this->v_area_description = $p_area_description;
	}
    ...
    /**
     * function return properties "area_order" value
     * @return int value
     */
    public function get_area_order(){
        return (int) $this->v_area_order;
    }


    /**
     * function allow change properties "area_order" value
     * @param $p_area_order: int value
     */
    public function set_area_order($p_area_order){
        $this->v_area_order = (int) $p_area_order;
    }


    /**
	 * function return properties "area_code" value
	 * @return string value
	 */
	public function get_area_code(){
		return $this->v_area_code;
	}

	
	/**
	 * function allow change properties "area_code" value
	 * @param $p_area_code: string value
	 */
	public function set_area_code($p_area_code){
		$this->v_area_code = $p_area_code;
	}
	...
	public function insert(){
		$arr = array('area_id' => $this->v_area_id
					,'company_id' => $this->v_company_id
					,'location_id' => $this->v_location_id
					,'area_name' => $this->v_area_name
					,'area_description' => $this->v_area_description
					,'status' => $this->v_status
					,'area_order' => $this->v_area_order
					,'area_code' => $this->v_area_code
        );
		try{
	 ...
	 * @example:
	 * <code>
	 *       SELECT * FROM `tb_area` WHERE `user_id`=2 ORDER BY `user_email` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_area($db)
	...
	public function select_one(array $arr_where = array(), array $arr_order = array()){
		...
		foreach($rss as $arr){
			...
			$this->v_area_name = isset($arr['area_name'])?$arr['area_name']:'';
			$this->v_area_description = isset($arr['area_description'])?$arr['area_description']:'';
			$this->v_status = isset($arr['status'])?$arr['status']:0;
			$this->v_area_code = isset($arr['area_code'])?$arr['area_code']:'';
			$this->v_area_order = isset($arr['area_order'])?$arr['area_order']:0;
	 ...
	 * @example: 
	 * <code>
	 * SELECT `area_id` FROM `tb_area` WHERE `user_id`=2 ORDER BY `user_email` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_area($db)
	 * 		 $cls->select_scalar('area_id',array('user_id'=>2), array('user_email'=>-1))
	 * </code>
	 * @return mixed
	 */
	 ...
	 * @example: 
	 * <code>
	 *   SELECT `area_id` FROM `tb_area` WHERE `user_id`=2 ORDER BY `area_id` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_area($db)
	 * 		 $cls->select_next('area_id',array('user_id'=>2), array('area_id'=>-1))
	 * </code>
	 * @return int
	 */
	...
	public function select_distinct($p_field_name){
		return $this->collection->command(array("distinct"=>"tb_area", "key"=>$p_field_name));
	}
	...
	public function update(array $arr_where = array()){
		...
		if(isset($v_has_mongo_id) && $v_has_mongo_id)
			$arr = array('$set' => array('area_id' => $this->v_area_id,'company_id' => $this->v_company_id,'location_id' => $this->v_location_id,'area_name' => $this->v_area_name,'area_description' => $this->v_area_description,'status' => $this->v_status,'area_code' => $this->v_area_code, 'area_order'=>$this->v_area_order));
		 else 
			$arr = array('$set' => array('company_id' => $this->v_company_id,'location_id' => $this->v_location_id,'area_name' => $this->v_area_name,'area_description' => $this->v_area_description,'status' => $this->v_status,'area_code' => $this->v_area_code, 'area_order'=>$this->v_area_order));
		try{
