...
class cls_tb_rule{
	...
	private $v_rule_id = 0;
	private $v_rule_title = '';
	private $v_rule_type = 0;
	private $v_rule_key = '';
	private $v_rule_admin = 0;
    private $v_rule_menu ='';
    private $v_rule_comp = 0;
    private $v_rule_description='';
	...
	public function __construct(MongoDB $db, $p_log_dir = ""){
		...
		$this->collection = $db->selectCollection('tb_rule');
		$this->collection->ensureIndex(array("rule_id"=>1), array('name'=>"rule_id_key", "unique"=>1, "dropDups" => 1));
	}
	...
	/**
	 * function return properties "rule_id" value
	 * @return int value
	 */
	public function get_rule_id(){
		return (int) $this->v_rule_id;
	}

	
	/**
	 * function allow change properties "rule_id" value
	 * @param $p_rule_id: int value
	 */
	public function set_rule_id($p_rule_id){
		$this->v_rule_id = (int) $p_rule_id;
	}

	
	/**
	 * function return properties "rule_title" value
	 * @return string value
	 */
	public function get_rule_title(){
		return $this->v_rule_title;
	}

	
	/**
	 * function allow change properties "rule_title" value
	 * @param $p_rule_title: string value
	 */
	public function set_rule_title($p_rule_title){
		$this->v_rule_title = $p_rule_title;
	}

	
	/**
	 * function return properties "rule_type" value
	 * @return int value
	 */
	public function get_rule_type(){
		return (int) $this->v_rule_type;
	}

	
	/**
	 * function allow change properties "rule_type" value
	 * @param $p_rule_type: int value
	 */
	public function set_rule_type($p_rule_type){
		$this->v_rule_type = (int) $p_rule_type;
	}

    /**
     * @param $p_rule_comp
     */
    public function set_rule_comp($p_rule_comp){
        $this->v_rule_comp = (int) $p_rule_comp;
    }

    /**
     * @return int
     */
    public  function get_rule_comp(){
        return (int) $this->v_rule_comp;
    }

	/**
	 * function return properties "rule_key" value
	 * @return string value
	 */
	public function get_rule_key(){
		return $this->v_rule_key;
	}

	
	/**
	 * function allow change properties "rule_key" value
	 * @param $p_rule_key: string value
	 */
	public function set_rule_key($p_rule_key){
		$this->v_rule_key = $p_rule_key;
	}

	
	/**
	 * function return properties "rule_admin" value
	 * @return int value
	 */
	public function get_rule_admin(){
		return (int) $this->v_rule_admin;
	}

	
	/**
	 * function allow change properties "rule_admin" value
	 * @param $p_rule_admin: int value
	 */
	public function set_rule_admin($p_rule_admin){
		$this->v_rule_admin = (int) $p_rule_admin;
	}
    ...
    public function get_rule_menu(){
        return $this->v_rule_menu;
    }

    /**
     * @param $p_rule_menu
     */
    public function set_rule_menu($p_rule_menu){
        $this->v_rule_menu = $p_rule_menu;
    }

    /**
     * @return mixed
     */
    public function get_rule_description(){
        return $this->v_rule_description;
    }

    /**
     * @param $p_rule_description
     */
    public function set_rule_description($p_rule_description)
    {
        $this->v_rule_description = $p_rule_description;
    }
	...
	public function insert(){
		$arr = array('rule_id' => $this->v_rule_id
					,'rule_title' => $this->v_rule_title
					,'rule_type' => $this->v_rule_type
					,'rule_key' => $this->v_rule_key
					,'rule_admin' => $this->v_rule_admin
					,'rule_menu' => $this->v_rule_menu
                    ,'rule_comp'=>$this->v_rule_comp
                    ,'rule_description'=>$this->v_rule_description
        );
		try{
	 ...
	 * @example:
	 * <code>
	 *       SELECT * FROM `tb_rule` WHERE `user_id`=2 ORDER BY `user_email` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_rule($db)
	...
	public function select_one(array $arr_where = array(), array $arr_order = array()){
		...
		$v_count = 0;
		foreach($rss as $arr){
			$this->v_rule_id = isset($arr['rule_id'])?$arr['rule_id']:0;
			$this->v_rule_title = isset($arr['rule_title'])?$arr['rule_title']:'';
			$this->v_rule_type = isset($arr['rule_type'])?$arr['rule_type']:0;
			$this->v_rule_key = isset($arr['rule_key'])?$arr['rule_key']:'';
			$this->v_rule_admin = isset($arr['rule_admin'])?$arr['rule_admin']:0;
			$this->v_rule_menu = isset($arr['rule_menu'])?$arr['rule_menu']:'';
			$this->v_rule_comp = isset($arr['rule_comp'])?$arr['rule_comp']:'0';
			$this->v_rule_description = isset($arr['rule_description'])?$arr['rule_description']:'';
	 ...
	 * @example: 
	 * <code>
	 * SELECT `rule_id` FROM `tb_rule` WHERE `user_id`=2 ORDER BY `user_email` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_rule($db)
	 * 		 $cls->select_scalar('rule_id',array('user_id'=>2), array('user_email'=>-1))
	 * </code>
	 * @return mixed
	 */
	 ...
	 * @example: 
	 * <code>
	 *   SELECT `rule_id` FROM `tb_rule` WHERE `user_id`=2 ORDER BY `rule_id` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_rule($db)
	 * 		 $cls->select_next('rule_id',array('user_id'=>2), array('rule_id'=>-1))
	 * </code>
	 * @return int
	 */
	...
	public function update(array $arr_where = array()){
		...
		if(isset($v_has_mongo_id) && $v_has_mongo_id)
			$arr = array('$set' => array('rule_id' => $this->v_rule_id,'rule_title' => $this->v_rule_title,'rule_type' => $this->v_rule_type,'rule_key' => $this->v_rule_key,'rule_admin' => $this->v_rule_admin,'rule_menu'=>$this->v_rule_menu,'rule_comp'=>$this->v_rule_comp,'rule_description'=>$this->v_rule_description));
		 else 
			$arr = array('$set' => array('rule_title' => $this->v_rule_title,'rule_type' => $this->v_rule_type,'rule_key' => $this->v_rule_key,'rule_admin' => $this->v_rule_admin,'rule_menu'=>$this->v_rule_menu,'rule_comp'=>$this->v_rule_comp,'rule_description'=>$this->v_rule_description));
		try{
