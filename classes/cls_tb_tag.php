...
class cls_tb_tag{
	...
	private $v_tag_id = 0;
	private $v_tag_name = '';
	private $v_tag_status = 0;
	private $v_tag_order = 0;
	...
	public function __construct(MongoDB $db, $p_log_dir = ""){
		...
		$this->collection = $db->selectCollection('tb_tag');
		$this->v_date_created = new MongoDate(time());
		$this->collection->ensureIndex(array("tag_id"=>1), array('name'=>"tag_id_key", "unique"=>1, "dropDups" => 1));
	}
	...
	/**
	 * function return properties "tag_id" value
	 * @return int value
	 */
	public function get_tag_id(){
		return (int) $this->v_tag_id;
	}

	
	/**
	 * function allow change properties "tag_id" value
	 * @param $p_tag_id: int value
	 */
	public function set_tag_id($p_tag_id){
		$this->v_tag_id = (int) $p_tag_id;
	}

	
	/**
	 * function return properties "tag_name" value
	 * @return string value
	 */
	public function get_tag_name(){
		return $this->v_tag_name;
	}

	
	/**
	 * function allow change properties "tag_name" value
	 * @param $p_tag_name: string value
	 */
	public function set_tag_name($p_tag_name){
		$this->v_tag_name = $p_tag_name;
	}

	
	/**
	 * function return properties "tag_status" value
	 * @return int value
	 */
	public function get_tag_status(){
		return (int) $this->v_tag_status;
	}

	
	/**
	 * function allow change properties "tag_status" value
	 * @param $p_tag_status: int value
	 */
	public function set_tag_status($p_tag_status){
		$this->v_tag_status = (int) $p_tag_status;
	}

	
	/**
	 * function return properties "tag_order" value
	 * @return int value
	 */
	public function get_tag_order(){
		return (int) $this->v_tag_order;
	}

	
	/**
	 * function allow change properties "tag_order" value
	 * @param $p_tag_order: int value
	 */
	public function set_tag_order($p_tag_order){
		$this->v_tag_order = (int) $p_tag_order;
	}
	...
	public function insert(){
		$arr = array('tag_id' => $this->v_tag_id
					,'tag_name' => $this->v_tag_name
					,'tag_status' => $this->v_tag_status
					,'tag_order' => $this->v_tag_order
	 ...
	 * @example:
	 * <code>
	 *       SELECT * FROM `tb_tag` WHERE `user_id`=2 ORDER BY `user_email` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_tag($db)
	...
	public function select_one(array $arr_where = array(), array $arr_order = array()){
		...
		$v_count = 0;
		foreach($rss as $arr){
			$this->v_tag_id = isset($arr['tag_id'])?$arr['tag_id']:0;
			$this->v_tag_name = isset($arr['tag_name'])?$arr['tag_name']:'';
			$this->v_tag_status = isset($arr['tag_status'])?$arr['tag_status']:0;
			$this->v_tag_order = isset($arr['tag_order'])?$arr['tag_order']:0;
	 ...
	 * @example: 
	 * <code>
	 * SELECT `tag_id` FROM `tb_tag` WHERE `user_id`=2 ORDER BY `user_email` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_tag($db)
	 * 		 $cls->select_scalar('tag_id',array('user_id'=>2), array('user_email'=>-1))
	 * </code>
	 * @return mixed
	 */
	 ...
	 * @example: 
	 * <code>
	 *   SELECT `tag_id` FROM `tb_tag` WHERE `user_id`=2 ORDER BY `tag_id` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_tag($db)
	 * 		 $cls->select_next('tag_id',array('user_id'=>2), array('tag_id'=>-1))
	 * </code>
	 * @return int
	 */
	...
	public function select_distinct($p_field_name){
		return $this->collection->command(array("distinct"=>"tb_tag", "key"=>$p_field_name));
	}
	...
	public function update(array $arr_where = array()){
		...
		if(isset($v_has_mongo_id) && $v_has_mongo_id)
			$arr = array('$set' => array('tag_id' => $this->v_tag_id,'tag_name' => $this->v_tag_name,'tag_status' => $this->v_tag_status,'tag_order' => $this->v_tag_order,'location_id' => $this->v_location_id,'company_id' => $this->v_company_id,'user_name' => $this->v_user_name,'date_created' => $this->v_date_created));
		 else 
			$arr = array('$set' => array('tag_name' => $this->v_tag_name,'tag_status' => $this->v_tag_status,'tag_order' => $this->v_tag_order,'location_id' => $this->v_location_id,'company_id' => $this->v_company_id,'user_name' => $this->v_user_name,'date_created' => $this->v_date_created));
		try{
