<?php
class cls_tb_user_log{

	private $v_log_id = 0;
	private $v_user_id = 0;
	private $v_user_password = '';
	private $v_log_ipaddress = '';
	private $v_log_url = '';
	private $v_log_datetime = '0000-00-00 00:00:00';
	...
	public function __construct(MongoDB $db, $p_log_dir = ""){
		...
		$this->collection = $db->selectCollection('tb_user_log');
		$this->collection->ensureIndex(array("user_id"=>1));
		$this->v_log_datetime = new MongoDate(time());
	...
	/**
	 * function return properties "log_ipaddress" value
	 * @return string value
	 */
	public function get_log_ipaddress(){
		return $this->v_log_ipaddress;
	}

	
	/**
	 * function allow change properties "log_ipaddress" value
	 * @param $p_log_ipaddress: string value
	 */
	public function set_log_ipaddress($p_log_ipaddress){
		$this->v_log_ipaddress = $p_log_ipaddress;
	}
	...
	/**
	 * function return properties "log_datetime" value
	 * @return int value indicates amount of seconds
	 */
	public function get_log_datetime(){
		return  $this->v_log_datetime->sec;
	}

	
	/**
	 * function allow change properties "log_datetime" value
	 * @param $p_log_datetime: string value format type: yyyy-mm-dd H:i:s
	 */
	public function set_log_datetime($p_log_datetime){
		$this->v_log_datetime = new MongoDate(strtotime($p_log_datetime));
	}
	...
	public function insert(){
		$arr = array('log_id' => $this->v_log_id
					...
					,'log_ipaddress' => $this->v_log_ipaddress
					,'log_url' => $this->v_log_url
					,'log_datetime' => $this->v_log_datetime);
		try{
	 ...
	 * @example:
	 * <code>
	 *       SELECT * FROM `tb_user_log` WHERE `user_id`=2 ORDER BY `user_email` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_user_log($db)
	...
	public function select_one(array $arr_where = array(), array $arr_order = array()){
		...
		foreach($rss as $arr){
			...
			$this->v_log_ipaddress = isset($arr['log_ipaddress'])?$arr['log_ipaddress']:'';
			$this->v_log_url = isset($arr['log_url'])?$arr['log_url']:'';
			$this->v_log_datetime = isset($arr['log_datetime'])?$arr['log_datetime']:(new MongoDate(time()));
	 ...
	 * @example: 
	 * <code>
	 * SELECT `log_id` FROM `tb_user_log` WHERE `user_id`=2 ORDER BY `user_email` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_user_log($db)
	 ...
	 * @example: 
	 * <code>
	 *   SELECT `log_id` FROM `tb_user_log` WHERE `user_id`=2 ORDER BY `log_id` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_user_log($db)
	...
	public function update(array $arr_where = array()){
		...
		if(isset($v_has_mongo_id) && $v_has_mongo_id)
			$arr = array('$set' => array('log_id' => $this->v_log_id,'user_id' => $this->v_user_id,'user_password' => $this->v_user_password,'log_ipaddress' => $this->v_log_ipaddress,'log_url' => $this->v_log_url,'log_datetime' => $this->v_log_datetime));

            $arr = array('$set' => array('user_id' => $this->v_user_id,'user_password' => $this->v_user_password,'log_ipaddress' => $this->v_log_ipaddress,'log_url' => $this->v_log_url,'log_datetime' => $this->v_log_datetime));
		try{
