...
class cls_tb_location_area{
	...
	public function __construct(MongoDB $db, $p_log_dir = ""){
		...
		$this->collection = $db->selectCollection('tb_location_area');
	...
	/**
	 * function return properties "area_image" value
	 * @return string value
	 */
	public function get_area_image(){
		return $this->v_area_image;
	}

	
	/**
	 * function allow change properties "area_image" value
	 * @param $p_area_image: string value
	 */
	public function set_area_image($p_area_image){
		$this->v_area_image = $p_area_image;
	}
	 ...
	 * @example:
	 * <code>
	 *       SELECT * FROM `tb_location_area` WHERE `user_id`=2 ORDER BY `user_email` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_location_area($db)
	...
	public function select_one(array $arr_where = array(), array $arr_order = array()){
		...
		foreach($rss as $arr){
			...
			$this->v_area_image = isset($arr['area_image'])?$arr['area_image']:'';
	 ...
	 * @example: 
	 * <code>
	 * SELECT `area_id` FROM `tb_location_area` WHERE `user_id`=2 ORDER BY `user_email` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_location_area($db)
	 ...
	 * @example: 
	 * <code>
	 *   SELECT `area_id` FROM `tb_location_area` WHERE `user_id`=2 ORDER BY `area_id` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_location_area($db)
	...
	public function update(array $arr_where = array()){
		...
		if(isset($v_has_mongo_id) && $v_has_mongo_id)
			$arr = array('$set' => array('area_id' => $this->v_area_id,'location_id' => $this->v_location_id,'area_name' => $this->v_area_name,'area_description' => $this->v_area_description,'area_image' => $this->v_area_image));
		 else 
			$arr = array('$set' => array('location_id' => $this->v_location_id,'area_name' => $this->v_area_name,'area_description' => $this->v_area_description,'area_image' => $this->v_area_image));
		try{
    ...
    public function delete_area_image($p_image_id, array $arr_where = array()){
        if(is_null($arr_where) || count($arr_where)==0){
            $v_has_mongo_id = !is_null($this->v_mongo_id);
            if($v_has_mongo_id)
                $arr_where = array('_id' => $this->v_mongo_id);
        ...
        try{
            ...
            $this->collection->update($arr_where,
                                        array('$pop'=>array('area_image'=>array('image_id'=>(int )$p_image_id))));
            return true;
        }catch(MongoCursorException $e){
            $this->v_error_code = $e->getCode();
            $this->v_error_message = $e->getMessage();
