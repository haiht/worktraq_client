...
class cls_tb_color{
	...
	private $v_color_id = 0;
	private $v_color_name = '';
	private $v_color_code = '';
	private $v_color_code_hex = '';
	private $v_color_order = 0;
	private $v_color_status = 0;
	...
	public function __construct(MongoDB $db, $p_log_dir = ""){
		...
		$this->collection = $db->selectCollection('tb_color');
		$this->collection->ensureIndex(array("color_name"=>1));
		$this->collection->ensureIndex(array("color_id"=>1), array('name'=>"color_id_key", "unique"=>1, "dropDups" => 1));
	}
	...
	/**
	 * function return properties "color_id" value
	 * @return int value
	 */
	public function get_color_id(){
		return (int) $this->v_color_id;
	}

	
	/**
	 * function allow change properties "color_id" value
	 * @param $p_color_id: int value
	 */
	public function set_color_id($p_color_id){
		$this->v_color_id = (int) $p_color_id;
	}

	
	/**
	 * function return properties "color_name" value
	 * @return string value
	 */
	public function get_color_name(){
		return $this->v_color_name;
	}

	
	/**
	 * function allow change properties "color_name" value
	 * @param $p_color_name: string value
	 */
	public function set_color_name($p_color_name){
		$this->v_color_name = $p_color_name;
	}

	
	/**
	 * function return properties "color_code" value
	 * @return string value
	 */
	public function get_color_code(){
		return $this->v_color_code;
	}

	
	/**
	 * function allow change properties "color_code" value
	 * @param $p_color_code: string value
	 */
	public function set_color_code($p_color_code){
		$this->v_color_code = $p_color_code;
	}

	
	/**
	 * function return properties "color_code_hex" value
	 * @return string value
	 */
	public function get_color_code_hex(){
		return $this->v_color_code_hex;
	}

	
	/**
	 * function allow change properties "color_code_hex" value
	 * @param $p_color_code_hex: string value
	 */
	public function set_color_code_hex($p_color_code_hex){
		$this->v_color_code_hex = $p_color_code_hex;
	}

	
	/**
	 * function return properties "color_order" value
	 * @return int value
	 */
	public function get_color_order(){
		return (int) $this->v_color_order;
	}

	
	/**
	 * function allow change properties "color_order" value
	 * @param $p_color_order: int value
	 */
	public function set_color_order($p_color_order){
		$this->v_color_order = (int) $p_color_order;
	}

	
	/**
	 * function return properties "color_status" value
	 * @return int value
	 */
	public function get_color_status(){
		return (int) $this->v_color_status;
	}

	
	/**
	 * function allow change properties "color_status" value
	 * @param $p_color_status: int value
	 */
	public function set_color_status($p_color_status){
		$this->v_color_status = (int) $p_color_status;
	}
	...
	public function insert(){
		$arr = array('color_id' => $this->v_color_id
					,'color_name' => $this->v_color_name
					,'color_code' => $this->v_color_code
					,'color_code_hex' => $this->v_color_code_hex
					,'color_order' => $this->v_color_order
					,'color_status' => $this->v_color_status);
		try{
	 ...
	 * @example:
	 * <code>
	 *       SELECT * FROM `tb_color` WHERE `user_id`=2 ORDER BY `user_email` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_color($db)
	...
	public function select_one(array $arr_where = array(), array $arr_order = array()){
		...
		$v_count = 0;
		foreach($rss as $arr){
			$this->v_color_id = isset($arr['color_id'])?$arr['color_id']:0;
			$this->v_color_name = isset($arr['color_name'])?$arr['color_name']:'';
			$this->v_color_code = isset($arr['color_code'])?$arr['color_code']:'';
			$this->v_color_code_hex = isset($arr['color_code_hex'])?$arr['color_code_hex']:'';
			$this->v_color_order = isset($arr['color_order'])?$arr['color_order']:0;
			$this->v_color_status = isset($arr['color_status'])?$arr['color_status']:0;
	 ...
	 * @example: 
	 * <code>
	 * SELECT `color_id` FROM `tb_color` WHERE `user_id`=2 ORDER BY `user_email` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_color($db)
	 * 		 $cls->select_scalar('color_id',array('user_id'=>2), array('user_email'=>-1))
	 * </code>
	 * @return mixed
	 */
	 ...
	 * @example: 
	 * <code>
	 *   SELECT `color_id` FROM `tb_color` WHERE `user_id`=2 ORDER BY `color_id` DESC LIMIT 0,1
	 * 		 $cls = new cls_tb_color($db)
	 * 		 $cls->select_next('color_id',array('user_id'=>2), array('color_id'=>-1))
	 * </code>
	 * @return int
	 */
	...
	public function update(array $arr_where = array()){
		...
		if(isset($v_has_mongo_id) && $v_has_mongo_id)
			$arr = array('$set' => array('color_id' => $this->v_color_id,'color_name' => $this->v_color_name,'color_code' => $this->v_color_code,'color_code_hex' => $this->v_color_code_hex,'color_order' => $this->v_color_order,'color_status' => $this->v_color_status));
		else
			$arr = array('$set' => array('color_name' => $this->v_color_name,'color_code' => $this->v_color_code,'color_code_hex' => $this->v_color_code_hex,'color_order' => $this->v_color_order,'color_status' => $this->v_color_status));
		try{
