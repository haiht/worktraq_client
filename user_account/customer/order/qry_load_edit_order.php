...
if($v_row==1){
    ...
    if(count($arr_tmp_allocation)<=0)
    {
        ...
        $tpl_one_allocation->set('TOTAL_PRICE',$v_user_rule_hide_price_all?NO_PRICE:'');
        ...
        foreach($arr_tmp_allocation as $v_location_id=>$arr)
        {
            ...
            $tpl_one_allocation->set('TOTAL_PRICE', $v_user_rule_hide_price_all?NO_PRICE:format_currency($v_allocation_total_amount+$v_location_total_amount));
            $tpl_one_allocation->set('TABLE_DISTRIBUTION_ITEM', $v_dsp_one_allocation);
            $arr_tpl_order_allocation[] = $tpl_one_allocation;
        }
    }
