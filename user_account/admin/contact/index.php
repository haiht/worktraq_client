...
//Right for User Module
$v_user_create_right = isset($arr_user_module_rule['manage_user']['create']);
$v_user_view_right = isset($arr_user_module_rule['manage_user']['view']);
...
switch($v_act){
	...
	case 'D':
		...
		break;
	case 'J':
		include 'qry_json_tb_contact.php';
		break;
    case 'AJ':
        $v_ajax_type = isset($_POST['txt_ajax_type'])?$_POST['txt_ajax_type']:'';
        if($v_ajax_type=='create_user')
            include 'qry_ajax_create_user.php';
        else if($v_ajax_type=='load_location')
            include 'qry_ajax_load_location.php';
        else if($v_ajax_type=='check_exist_user')
            include 'qry_ajax_check_exist_user.php';
        break;
	case 'X':
		if($v_report_right || $v_is_admin){
			include 'qry_export_tb_contact.php';
		}else{
			redir(URL.'admin/error/');
		}
		break;
	case 'P':
		if($v_report_right || $v_is_admin){
			include 'qry_print_tb_contact.php';
			include 'user_account/admin/print_header.php';
			include 'dsp_print_tb_contact.php';
