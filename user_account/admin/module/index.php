...
add_class('cls_tb_module', 'cls_tb_module.php');
...
switch($v_act){
	case 'N':
		if($v_create_right || $v_is_admin){
			include 'qry_single_tb_module.php';
			include 'user_account/admin/admin_header.php';
			include 'dsp_single_tb_module.php';
	...
	case 'D':
		if($v_delete_right || $v_is_admin){
			include 'act_delete_tb_module.php';
		}else{
			redir(URL.'admin/error/');
		}
		break;
	case 'J':
	    include 'qry_json_tb_module.php';
		break;
    case 'AJ':
        $v_ajax_type = isset($_POST['txt_ajax_type'])?$_POST['txt_ajax_type']:'';
        if($v_ajax_type=='load_index')
            include 'qry_ajax_load_index.php';
        else if($v_ajax_type=='check_module_menu')
            include 'qry_ajax_check_module_menu.php';
        break;
	case 'X':
		if($v_report_right || $v_is_admin){
			include 'qry_export_tb_module.php';
		}else{
			redir(URL.'admin/error/');
		}
		break;
	case 'P':
		if($v_report_right || $v_is_admin){
			include 'qry_print_tb_module.php';
			include 'user_account/admin/print_header.php';
			include 'dsp_print_tb_module.php';
	...
	default:
		if($v_view_right || $v_is_admin){
			$v_act = 'A';
			include 'qry_all_tb_module.php';
			include 'user_account/admin/admin_header.php';
			include 'dsp_all_tb_module.php';
			include 'user_account/admin/admin_footer.php';
		}else{
			redir(URL.'admin/error/');
		}
