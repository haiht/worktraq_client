...
if($v_flag!=''){
    if($v_product_images_id>0){
        ...
        if($v_row==1){
            ...
            $arr_image = $cls_tb_product_images->get_map_images();
            if($v_flag=='delete'){
                $v_ret = $cls_tb_product_images->delete(array('product_images_id'=>$v_product_images_id));
                if($v_ret){
                    ...
                    //    if(file_exists($arr_image[$i])) @unlink($arr_image[$i]);
                    //}
                ...
                $v_ret = $cls_tb_product_images->update_field('status', $v_status, array('product_images_id'=>$v_product_images_id));
                if($v_ret)
                ...
                $v_ret = $cls_tb_product_images->update_field('image_type', $v_status, array('product_images_id'=>$v_product_images_id));
                if($v_ret)
                    die(json_encode(array('error'=>0, 'message'=>'Change status success')));
                else
                    die(json_encode(array('error'=>6, 'message'=>'Change status not success')));
            }else if($v_flag=='savecode'){
                $v_ret = $cls_tb_product_images->update_field('image_code', $v_code, array('product_images_id'=>$v_product_images_id));
                if($v_ret)
