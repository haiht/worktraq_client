...
if(isset($_POST['btn_submit_tb_product'])){
    ...
    $arr_product_tag = isset($_POST['txt_tag_id'])?$_POST['txt_tag_id']:$arr_product_tag;
...
$arr_all_group = get_array_data($cls_tb_product_group, 'product_group_id', 'product_group_name', $v_tmp_product_threshold_group_id, array(0,'--------'), array('company_id'=>$v_company_id));
...
for($i=0; $i<count($arr_material); $i++){
    ...
    $v_dsp_material_list .= '<div class="node k-block k-widget" id="material_'.$id.'"><div style="width:10px; height:10px;border: 1px solid #000; background-color:'.$c.'"></div>'.$m.' ('.$w.' &times; '.$l.' '.$us.') '.$t. ' '.$ut.' &rArr; $'.$p;
    $v_dsp_material_list .= '<img class="img_action" id="img_material_'.$id.'" data-id="'.$id.'" data-name="'.$m.'" data-width="'.$w.'" data-length="'.$l.'" data-unit-size="'.$us.'" data-thick="'.$t.'" data-unit-thick="'.$ut.'" data-price="'.$p.'" data-color="'.$c.'" data-size="'.$size.'" data-sided="'.$sided.'" data-flag="material" src="images/icons/delete.png" />';
