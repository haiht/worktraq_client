...
add_class('cls_tb_product_group', 'cls_tb_product_group.php');
$cls_tb_product_group = new cls_tb_product_group($db, LOG_DIR);

add_class('cls_tb_product', 'cls_tb_product.php');
$cls_tb_product = new cls_tb_product($db, LOG_DIR);
add_class('cls_tb_location_group_threshold');
$cls_tb_location_group_threshold = new cls_tb_location_group_threshold($db, LOG_DIR);
...
switch($v_act){
	...
	case 'J':
        ...
        if($v_json_type=='load_product_group'){
		    include 'qry_json_tb_product_group.php';
        }else if($v_json_type=='load_product'){
            include 'qry_json_load_tb_product.php';
        }else if($v_json_type=='load_location_group_threshold'){
            include 'qry_json_load_tb_location_group_threshold.php';
        }
		break;
	case 'X':
		if($v_report_right || $v_is_admin){
			include 'qry_export_tb_product_group.php';
		}else{
			redir(URL.'admin/error/');
		}
		break;
	case 'AJ':
        ...
        if($v_ajax_type=='load_threshold_info')
            include 'qry_ajax_load_threshold_info.php';
		break;
	case 'P':
		if($v_report_right || $v_is_admin){
			include 'qry_print_tb_product_group.php';
			include 'user_account/admin/print_header.php';
			include 'dsp_print_tb_product_group.php';
