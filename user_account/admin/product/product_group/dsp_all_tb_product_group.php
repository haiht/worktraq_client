        ...
        <div id="div_splitter_content" style="height: 100%; width: 100%;">
            ...
            <div id="div_right_pane">
                <div class="pane-content">
                    ...
                    <input type="text" name="txt_quick_search" placeholder="Search by Product Group Name" value="<?php echo isset($v_quick_search)?htmlspecialchars($v_quick_search):'';?>" />
                    ...
                    <h2>Advanced Search for Product_group</h2>
				...
				<script type="text/javascript">
                    ...
                    $(document).ready(function() {
                        ...
                        $('li#icons_advanced_search').bind("click", function() {
                            if (!window_search.data("kendoWindow")) {
                                window_search.kendoWindow({
                                    ...
                                    modal: true,
                                    title: "Advanced Search for Product_group"
                                });
                            }
                            ...
                            dataSource: {
                                ...
                                transport: {
                                    read: {
                                        ...
                                        type: "POST",
                                        data: {txt_session_id:"<?php echo session_id();?>",txt_json_type:'load_product_group', txt_quick_search:'<?php echo isset($v_quick_search)?htmlspecialchars($v_quick_search):'';?>',txt_search_product_group_name:'<?php echo isset($v_search_product_group_name)?htmlspecialchars($v_search_product_group_name):'';?>', txt_search_company_id: '<?php echo $v_company_id;?>'}
                                    }
                                },
                                schema: {
                                    data: "tb_product_group"
                                    ,total: function(data){
						...
						columns: [
							...
							{field: "product_group_name", title: "Product Group Name", type:"string", width:"100px", sortable: true },
							{field: "product_group_order", title: "Group Order", type:"int", width:"50px", sortable: true, template: '<span style="float:right">#= kendo.toString(product_group_order,"n0") #</span>'},
                            {field: "product_group_overflow", title: "Overflow Allow", type:"boolean", width:"50px", sortable: true, template: '<span style="float:right">#= product_group_overflow==1?"Yes":"No" #</span>'},
							{field: "product_group_parent", title: "Parent Group", type:"string", width:"80px", sortable: true},
							{ command:  [
              ...
              function view_row(e) {
                    ...
                    document.location = "<?php echo URL.$v_admin_key.'/';?>"+dataItem.product_group_id+"/view";
                }
                function edit_row(e) {
                    e.preventDefault();
                    var dataItem = this.dataItem($(e.currentTarget).closest("tr"));
                    if(confirm('Do you want to edit product_group with ID: '+dataItem.product_group_id+'?')){
                        document.location = "<?php echo URL.$v_admin_key.'/';?>"+dataItem.product_group_id+"/edit";
                    }
                }
                function delete_row(e) {
                    e.preventDefault();
                    var dataItem = this.dataItem($(e.currentTarget).closest("tr"));
                    if(confirm('Do you want to delete product_group with ID: '+dataItem.product_group_id+'?')){
                        document.location = "<?php echo URL.$v_admin_key.'/';?>"+dataItem.product_group_id+"/delete";
                    }
                }
            </script>
                </div>
