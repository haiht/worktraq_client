...
if(isset($_POST['btn_submit_tb_material'])){
	...
	$cls_tb_material->set_size_option($v_size_option);
	$v_two_sided_print = isset($_POST['txt_two_sided_print'])?1:0;
	$cls_tb_material->set_two_sided_print($v_two_sided_print);
	$v_material_type = isset($_POST['txt_material_type'])?$_POST['txt_material_type']:$v_material_type;
	$v_material_type = (int) $v_material_type;
	if($v_material_type<0) $v_error_message .= '[Material Type] is negative!<br />';
	$cls_tb_material->set_material_type($v_material_type);
	$v_status = isset($_POST['txt_status'])?1:0;
	...
	$cls_tb_material->set_status($v_status);
	if($v_error_message==''){
		if(is_null($v_mongo_id)){
			$v_mongo_id = $cls_tb_material->insert();
			$v_result = is_object($v_mongo_id);
		}else{
			$v_result = $cls_tb_material->update(array('_id' => $v_mongo_id));
			$v_new_material = false;
		}
		if($v_result){
			$_SESSION['ss_tb_material_redirect'] = 1;
			redir(URL.$v_admin_key);
		}else{
			if($v_new_material) $v_material_id = 0;
		}
	}
}else{
	...
	if($v_material_id>0){
		...
		if($v_row == 1){
			...
			$v_status = $cls_tb_material->get_status();
		}
	}
}
...
for($i=0; $i<count($arr_material_option);$i++){
    ...
    $v_dsp_option .= '<p class="k-block k-widget"><img style="cursor:pointer" src="'.URL.'images/icons/delete.png" data-option="'.$v_data.'" id="img_action" title="Remove this option" />&nbsp;<label><input type="checkbox" data-option="'.$v_data.'" id="txt_option_status"'.($v_status==0?' checked="checked"':'').' />Active</label>';
