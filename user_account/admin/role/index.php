...
switch($v_act){
	...
	case 'D':
		...
		break;
	case 'J':
		include 'qry_json_tb_role.php';
		break;
    case 'AJ':
        $v_ajax_type = isset($_POST['txt_ajax_type'])?$_POST['txt_ajax_type']:'';
        if($v_ajax_type=='load_location'){
            include 'qry_ajax_load_location.php';
        }else if($v_ajax_type=='check_role_key'){
            include 'qry_ajax_check_role_key.php';
        }
        break;
	case 'X':
		if($v_report_right || $v_is_admin){
			include 'qry_export_tb_role.php';
		}else{
			redir(URL.'admin/error/');
		}
		break;
	case 'P':
		if($v_report_right || $v_is_admin){
			include 'qry_print_tb_role.php';
			include 'user_account/admin/print_header.php';
			include 'dsp_print_tb_role.php';
