...
$v_email_templates_id = isset($_GET['id'])?$_GET['id']:'0';
settype($v_email_templates_id, 'int');
add_class('cls_tb_email_templates', 'cls_tb_email_templates.php');
$cls_tb_email_templates = new cls_tb_email_templates($db, LOG_DIR);
...
switch($v_act){
	...
	case 'D':
		...
		break;
	case 'J':
		include 'qry_json_tb_email_templates.php';
		break;
	case 'X':
		if($v_report_right || $v_is_admin){
			include 'qry_export_tb_email_templates.php';
		}else{
			redir(URL.'admin/error/');
		}
		break;
	case 'AJ':
        ...
        if($v_ajax_type=='check_email_key')
            include 'qry_ajax_check_email_key.php';
		break;
	case 'P':
		if($v_report_right || $v_is_admin){
			include 'qry_print_tb_email_templates.php';
			include 'user_account/admin/print_header.php';
			include 'dsp_print_tb_email_templates.php';
