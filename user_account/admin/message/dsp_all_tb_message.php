        ...
        <div id="div_splitter_content" style="height: 100%; width: 100%;">
            ...
            <div id="div_right_pane">
                <div class="pane-content">
                    ...
                    <input type="text" name="txt_quick_search" placeholder="Search by Message Key or Message Value" value="<?php echo isset($v_quick_search)?htmlspecialchars($v_quick_search):'';?>" />
                    ...
                    <h2>Advanced Search for Message</h2>
				...
				<script type="text/javascript">
                    ...
                    $(document).ready(function() {
                        ...
                        $('li#icons_advanced_search').bind("click", function() {
                            if (!window_search.data("kendoWindow")) {
                                window_search.kendoWindow({
                                    ...
                                    modal: true,
                                    title: "Advanced Search for Message"
                                });
                            }
                            ...
                            dataSource: {
                                ...
                                transport: {
                                    read: {
                                        ...
                                        type: "POST",
                                        data: {txt_session_id:"<?php echo session_id();?>",txt_quick_search:'<?php echo isset($v_quick_search)?htmlspecialchars($v_quick_search):'';?>', txt_search_company_id: '<?php echo $v_company_id;?>', txt_search_message_key:'<?php echo isset($v_search_message_key)?htmlspecialchars($v_search_message_key):'';?>', txt_search_message_value:'<?php echo isset($v_search_message_value)?htmlspecialchars($v_search_message_value):'';?>'}
                                    }
                                },
                                schema: {
                                    data: "tb_message"
						...
						columns: [
							...
							{field: "message_key", title: "Message Key", type:"string", width:"100px", sortable: true },
							{field: "message_value", title: "Message Value", type:"string", width:"150px", sortable: true },
                            {field: "message_type", title: "Message Type", type:"int", width:"50px", sortable: true, template: '<span style="float:right">#= kendo.toString(message_type,"n0") #</span>'},
							{field: "message_order", title: "Message Order", type:"string", width:"50px", sortable: true },
							{ command:  [
              ...
              function view_row(e) {
                    ...
                    document.location = "<?php echo URL.$v_admin_key.'/';?>"+dataItem.message_id+"/view";
                }
                function edit_row(e) {
                    e.preventDefault();
                    var dataItem = this.dataItem($(e.currentTarget).closest("tr"));
                    if(confirm('Do you want to edit message with ID: '+dataItem.message_id+'?')){
                        document.location = "<?php echo URL.$v_admin_key.'/';?>"+dataItem.message_id+"/edit";
                    }
                }
                function delete_row(e) {
                    e.preventDefault();
                    var dataItem = this.dataItem($(e.currentTarget).closest("tr"));
                    if(confirm('Do you want to delete message with ID: '+dataItem.message_id+'?')){
                        document.location = "<?php echo URL.$v_admin_key.'/';?>"+dataItem.message_id+"/delete";
                    }
                }
            </script>
                </div>
