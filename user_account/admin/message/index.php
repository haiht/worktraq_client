...
add_class('cls_tb_message', 'cls_tb_message.php');
...
switch($v_act){
	...
	case 'D':
		...
		break;
	case 'J':
		include 'qry_json_tb_message.php';
		break;
	case 'X':
		if($v_report_right || $v_is_admin){
			include 'qry_export_tb_message.php';
		}else{
			redir(URL.'admin/error/');
		}
		break;
	case 'AJ':
        ...
        if($v_ajax_type=='check_message_key')
            include 'qry_ajax_check_message_key.php';
		break;
	case 'P':
		if($v_report_right || $v_is_admin){
			include 'qry_print_tb_message.php';
			include 'user_account/admin/print_header.php';
			include 'dsp_print_tb_message.php';
