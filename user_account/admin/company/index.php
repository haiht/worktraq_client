...
switch($v_act){
	...
	case 'D':
		...
		break;
	case 'J':
		include 'qry_json_tb_company.php';
		break;
	case 'X':
		if($v_report_right || $v_is_admin){
			include 'qry_export_tb_company.php';
		}else{
			redir(URL.'admin/error/');
		}
		break;
	case 'AJ':
        ...
        if($v_ajax_type=='check_company_code')
            include 'qry_ajax_check_company_code.php';
		break;
	case 'P':
		if($v_report_right || $v_is_admin){
			include 'qry_print_tb_company.php';
			include 'user_account/admin/print_header.php';
			include 'dsp_print_tb_company.php';
