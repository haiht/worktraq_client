...
$v_settings_id = isset($_GET['id'])?$_GET['id']:'0';
settype($v_settings_id, 'int');
...
switch($v_act){
	case 'N':
		if($v_create_right || $v_is_admin){
			include 'qry_single_tb_settings.php';
			include 'user_account/admin/admin_header.php';
			include 'dsp_single_tb_settings.php';
	...
	case 'D':
		if($v_delete_right || $v_is_admin){
			include 'act_delete_tb_settings.php';
		}else{
			redir(URL.'admin/error/');
		}
		break;
	case 'J':
		include 'qry_json_tb_settings.php';
		break;
	case 'X':
		if($v_report_right || $v_is_admin){
			include 'qry_export_tb_settings.php';
		}else{
			redir(URL.'admin/error/');
		}
		break;
	case 'AJ':
        ...
        if($v_ajax_type=='check_setting_name')
            include 'qry_ajax_check_setting_name.php';
		break;
	case 'P':
		if($v_report_right || $v_is_admin){
			include 'qry_print_tb_settings.php';
			include 'user_account/admin/print_header.php';
			include 'dsp_print_tb_settings.php';
	...
	default:
		if($v_view_right || $v_is_admin){
			$v_act = 'A';
			include 'qry_all_tb_settings.php';
			include 'user_account/admin/admin_header.php';
			include 'dsp_all_tb_settings.php';
			include 'user_account/admin/admin_footer.php';
		}else{
			redir(URL.'admin/error/');
		}
