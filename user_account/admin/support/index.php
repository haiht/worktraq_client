...
add_class('cls_tb_support', 'cls_tb_support.php');
$cls_tb_support = new cls_tb_support($db, LOG_DIR);
...
switch($v_act){
	...
	case 'J':
        ...
        if($v_json_type=='load_support')
		    include 'qry_json_tb_support.php';
	...
	case 'X':
		if($v_report_right || $v_is_admin){
			include 'qry_export_tb_support.php';
		}else{
			redir(URL.'admin/error/');
		}
		break;
	case 'AJ':
        ...
        if($v_ajax_type=='load_location_contact')
            include 'qry_ajax_load_location_contact.php';
		break;
	case 'P':
		if($v_report_right || $v_is_admin){
			include 'qry_print_tb_support.php';
			include 'user_account/admin/print_header.php';
			include 'dsp_print_tb_support.php';
