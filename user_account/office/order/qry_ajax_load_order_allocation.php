...
if($v_request_order_id==0){
    if(isset($_SESSION['ss_current_order'])){
        ...
        for($i=0; $i<count($arr_order);$i++){
            ...
            if($v_product_id==$v_request_product_id){
                $arr_location[$v_idx] = array('product_id'=>$v_product_id, 'product_detail'=>$v_product_name, 'product_quantity'=>$v_product_quantity, 'product_price'=>$v_product_price, 'location'=>array(), 'allocation'=>array());
                $arr_allocation = $arr_order[$i]['allocation'];
                for($j=0;$j<count($arr_allocation); $j++){
                    $v_location_id = $arr_allocation[$j]['location_id'];
                    $v_location_name = $arr_allocation[$j]['location_name'];
                    $v_location_quantity = $arr_allocation[$j]['location_quantity'];
                    $v_location_price = $arr_allocation[$j]['location_price'];
                    if($v_location_quantity>0){
                        $arr_location[$v_idx]['location'][] = array('location_id'=>$v_location_id, 'location_name'=>$v_location_name, 'location_quantity'=>$v_location_quantity, 'location_price'=>$v_location_price);
                    }
                    if(strpos($v_product_excluded_location,$v_location_id.',')===false)//this location not in excluded list
                        $arr_location[$v_idx]['allocation'][] = array('location_id'=>$v_location_id, 'location_name'=>$v_location_name, 'location_number'=>$cls_location->select_scalar('location_number', array('location_id'=>$v_location_id)));
                }
                $v_idx++;
            }
        }
    ...
    if($v_request_order_item_id>0){
        ...
        if($v_row==1){
            ...
            for($i=0; $i<count($arr_allocation); $i++){
                ...
                //$arr_location[0]['location'][$i] = array('location_id'=>$v_location_id, 'location_name'=>$v_location_name, 'location_quantity'=>$v_location_quantity, 'location_price'=>$v_location_price);
                //$arr_location[0]['allocation'] = $arr_tmp_allocation;
            }
    ...
    //die('{error=2}{message='.$v_order_id.'}');
}
?>