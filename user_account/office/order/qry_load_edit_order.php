...
if($v_row==1){
    ...
    $tpl_order_information_form = new Template('dsp_order_information_form.tpl', $v_dir_templates);
    ...
    foreach($arr_order_items as $arr){
        ...
        //die('iTEM: '.$v_order_item_id);
        if($v_package_type<=1) {
            $v_product_material = $v_product_code .'<br>'.$v_product_description.'<br>';
            if ($v_material_name != '') {
            ...
            //$v_product_material = $v_product_code .'<br>'. $v_product_description.'<br>'. $v_material_name.'( '.$v_width.' &times; '.$v_length.' '.$v_unit.')' .$v_material_thickness_value . ' ' .$v_material_thickness_unit;
        }
        else
            $v_product_material = $v_product_code .'<br>'. $v_product_description.'<br>'.'( '.$cls_settings->get_option_name_by_id('package_type', $v_package_type).')';
        if($v_item_status==1){
            $v_order_flag = 1;
            //$v_product_material.='<br />[<span style="text-decoration:blink; color:red; font-weight: bold">'.$v_item_desc.'</span>]';
        }
        ...
        if($v_order=='EDIT'){
            ...
            if($v_status>=20){
                ...
                if($v_user_rule_edit || ($v_order_user_name==$arr_user['user_name']))
                    $v_dsp_order_option .= '<option value="delete">Delete</option><option value="edit">Edit</option>';
                if($v_user_rule_allocate)
                    $v_dsp_order_option .= '<option value="allocation">Allocate</option>';
            }
            $tpl_order_items->set('ORDER_OPTION_ACTION', $v_dsp_order_option);
        }else{
            $tpl_order_items->set('SELECT_DISABLED', 'disabled="disabled"');
        }
    ...
    foreach($arr_tmp_allocation as $v_location_id=>$arr){
        ...
        for($i=0; $i<count($arr); $i++){
            $tpl_order_allocation_items = new Template('dsp_order_allocation_items.tpl',$v_dir_templates);
            ...
            $v_allocation_total_amount += $v_tmp_location_price;
        ...
        $tpl_one_allocation->set('ORDER_ROW_ITEM_ALLOCATION', $v_dsp_one_allocation);
    ...
    //$fp = fopen('xem.txt', 'w');
    //$v_str = json_encode($arr_user_allocation);
    ...
    $tpl_order->set('JAVASCRIPT_TPL', $tpl_script->output());
    ...
    if($v_order=='EDIT'){
        //$v_order_information = 'You are working on order with PO NUMBER: '.$v_po_number.'. Status of this order is: \''.$v_order_status_message.'\'';
        //if($v_order_flag){
        //    $v_order_information.='<br /> Maybe one of order\'s items has not allocated. Please see tab \'ITEMS\' for more information.';
        //}
        //if($v_order_status<2){
        //    $v_order_information.='<br />Check your order and submit it, please';
        //}
    ...
    /*
    $v_error_approve = '';
    if(isset($_SESSION['ss_error_approved'])){
        $v_error_approve = $_SESSION['ss_error_approved'];
        $arr_error_approve = unserialize($v_error_approve);
        if(is_array($arr_error_approve)){
            $v_error_approve = isset($arr_error_approve[$v_order_id])?$arr_error_approve[$v_order_id]:'';
        }
    }
    if($v_error_approve==''){
        $v_error_approve = $v_order_notes;
    }
    */
    ...
    $tpl_order->set('ORDER_INFORMATION_FORM', $tpl_order_information_form->output());
