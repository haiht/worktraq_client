...
if(isset($_SESSION['ss_current_order'])){
    ...
    if(is_array($arr_order)){
        ...
        for($i=0; $i<count($arr_order);$i++){
            ...
            if($v_product_image!='' && strpos($v_product_image,'/')===false) $v_product_image = $v_company_product_url.$v_product_image;
            ...
            //$tpl_order_items->set('PRODUCT_IMAGE',URL.'images/products/'.$v_product_id.'/'.$v_product_image);
            $tpl_order_items->set('PRODUCT_IMAGE',$v_product_image);
            $tpl_order_items->set('PRODUCT_QUANTITY', $v_product_quantity);
            $tpl_order_items->set('PRODUCT_PRICE', $v_user_rule_hide_price_all?NO_PRICE:format_currency($v_product_price));
            $tpl_order_items->set('PRODUCT_AMOUNT', $v_user_rule_hide_price_all?NO_PRICE:format_currency($v_product_price*$v_product_quantity));
        ...
        foreach($arr_allocation_item as $v_location_id=>$arr){
            if($v_location_id!=$v_tmp_location){
                ...
                for($i=0; $i<count($arr); $i++){
                    ...
                    if($v_image_url!='' && strpos($v_image_url,'/')===false) $v_image_url = $v_company_product_url.$v_image_url;
                ...
                $tpl_one_allocation->set('LOCATION_PRICE', $v_user_rule_hide_price_all?NO_PRICE:format_currency($v_location_price));
        ...
        $tpl_order_information_form->set('DATE_REQUIRED',$v_date_required);
        ...
        $tpl_order->set('TOTAL_AMOUNT', $v_user_rule_hide_price_all?NO_PRICE:format_currency($v_total_amount));
        ...
        $tpl_script->set('COMPANY_PRODUCT_URL',$v_company_product_url);
