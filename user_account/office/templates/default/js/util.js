(function (jQuery) {
	...
	jQuery.fn.showPopup = function (options) {
		...
		return this.each(function () {
			var jpopup = jQuery(this).removeClass('hidden'),
			jwindow = jQuery(window),
			jhtml = jQuery(jQuery.browser.msie ? 'body' : 'html'),
			initScrollTop = jwindow.scrollTop(),
			overlay = null,
			opt = jQuery.extend(defaults, options);
			jpopup.appendTo(document.body);
			//load_ajax(opt.product_id, opt.order_id);
			if (!opt.noOverlay) {
				overlay = jQuery('#zoverlay');
				if (!overlay.length) {
					overlay = jQuery('<div id="zoverlay"></div>').css({
							'display' : 'block',
							'visibility' : 'visible',
							'position' : 'fixed',
							'top' : 0,
			...
			var rePosition = function () {
				...
				if (opt.dockPanel) {
					...
					case 5: //top header
						top = (jQuery('.header-bottom').offset().top + opt.dockOffset);
						// left = Math.max(0, winWidth - popupWidth - opt.dockOffset);
						break;
			...
			windowScroll = function () {
				if (opt.el) {
					...
					if (jQuery.browser.msie && parseInt(jQuery.browser.version) < 7) {
						...
						jpopup.css({
							'position' : 'absolute',
							'top' : newpos.top + jQuery(window).scrollTop()
						});
			...
			windowResize = function () {
				...
				if (overlay) {
					overlay.css({
						...
						'height' : Math.max(jwindow.height(), jQuery(jQuery.browser.msie ? (parseInt(jQuery.browser.version) >= 8 ? document : 'body') : 'html').height())
					});
				}
			},
			closePopup = function () {
				...
				if (jQuery.browser.msie && parseFloat(jQuery.browser.version) < 9) {
					setTimeout(function () {
						...
						if (opt.dockPanel == 5) {
							jQuery('.header-bottom-inner').removeClass('hidden');
						}
					}, opt.popupDuration / 3);
			...
			confirmPopup = function () {
				if (opt.onConfirm) {
					opt.onConfirm.call(jpopup);
				}
				/*DON'T REMOVE FOLLOWING STATEMENT */
				if(!control_popup) return false;
				/*END*/		
			...
			var submitBtn = jpopup.find(opt.submitButtons[0]);
			
			if (submitBtn.length) {			
				submitBtn.unbind('click.jpopupev').bind('click.jpopupev', confirmPopup);	
			}
